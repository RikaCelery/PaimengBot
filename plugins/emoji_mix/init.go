...
func init() {
	...
	mixCommandLimiter = rate.NewManager[string](time.Second*20, int(proxy.GetConfigInt64("command_limiter")))
	proxy.SetUsage(fmt.Sprintf(helpTemplate, proxy.GetConfigInt64("mix_limiter"), proxy.GetConfigInt64("command_limiter"), proxy.GetConfigInt64("command_limiter")))
	proxy.AddOnConfigChange(func() {
		mixLimiter = rate.NewManager[string](time.Minute*30, int(proxy.GetConfigInt64("mix_limiter")))
		mixCommandLimiter = rate.NewManager[string](time.Second*20, int(proxy.GetConfigInt64("command_limiter")))
		proxy.SetUsage(fmt.Sprintf(helpTemplate, proxy.GetConfigInt64("mix_limiter"), proxy.GetConfigInt64("command_limiter"), proxy.GetConfigInt64("command_limiter")))
	})
...
const (
	animated = `https://fonts.gstatic.com/s/e/notoemoji/latest/%s/512.gif`
	bed      = "https://www.gstatic.com/android/keyboard/emojikitchen/%s/%s/%s_%s.png"
)
...
func animeEmojiHandle(ctx *zero.Ctx) {
	...
	if len(found) > 0 {
		...
		log.Infof("<emoji_mix>local animated sticker not found (%s)", slug)
	}
...
func getLimiter(ctx *zero.Ctx) *rate.Limiter {
	...
	return mixLimiter.Load(fmt.Sprintf("%d", ctx.Event.GroupID))
}
func checkLimiter(ctx *zero.Ctx) bool {
	if getLimiter(ctx).Acquire() {
		return true
	}
	return false
}
...
func match(ctx *zero.Ctx) bool {
	...
	for _, emoji := range ctx.State["emojimix"].([]string) {
	...
	for _, segment := range msg {
		...
		case "text":
			...
			if len(loc) == 0 || loc[0][0] != 0 || loc[len(loc)-1][1] != len(s) {
				loc = emojiRx2.FindAllStringIndex(s, -1)
				regex2 = true
			}
			if len(loc) == 0 || loc[0][0] != 0 || loc[len(loc)-1][1] != len(s) {
				return
			...
			if regex2 {
				emojis = append(emojis, emojiRx2.FindAllString(segment.Data["text"], -1)...)
			} else {
...
func emojiToHashSlug(emoji string) (hashSlug string, err error) {
	if !emojiRx.MatchString(emoji) && !emojiRx2.MatchString(emoji) {
