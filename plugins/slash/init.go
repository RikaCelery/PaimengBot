...
var (
	info = manager.PluginInfo{
		Name: "回应动作",
		Usage: `slash Plugin, Origin from https://github.com/Rongronggg9/SlashBot
响应诸如 “/打”、“/敲” “/prpr” 之类消息
用法：
	/打
	/prpr @某人
...
`,
	...
	engine = manager.RegisterPlugin(info)
)
...
func init() {
	...
	*/
	engine.OnMessage(zero.NewPattern(nil).Text(`^/(\S{1,5})$`).At().AsRule()).SetPriority(9999).SetBlock(true).Handle(func(ctx *zero.Ctx) {
	...
	engine.OnMessage(zero.NewPattern(nil).Text(`^/(\S{1,5})$`).AsRule(), func(ctx *zero.Ctx) bool {
		model := extension.PatternModel{}
		_ = ctx.Parse(&model)
		msg := model.Matched[0].Text()[1]
		if p := manager.Lookup(msg); p != nil {
			return false
		}
		ctx.State["rua"] = msg
		return true
	}).SetPriority(9999).SetBlock(true).Handle(func(ctx *zero.Ctx) {
