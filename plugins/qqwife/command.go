...
var (
	...
	{cmd}[允许|禁止]自由恋爱
	{cmd}[允许|禁止]牛头人
	{cmd}设置CD为xx小时    →(默认12小时)
	{cmd}重置花名册
	{cmd}重置所有花名册(用于清除所有群数据及其设置)
	{cmd}查好感度[对方Q号|@对方QQ]
	{cmd}好感度列表
	{cmd}好感度数据整理 (当好感度列表出现重复名字时使用)
--------------------------------
...
func init() {
	engine.OnCommands([]string{"娶群友"}, zero.OnlyGroup, getdb).SetBlock(true).
	...
	engine.OnCommands([]string{"群老婆列表"}, zero.OnlyGroup, getdb).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			gid := ctx.Event.GroupID
			err := 民政局.开门时间(gid)
			if err != nil {
			...
			canvas := images.NewImageCtx(1500, int(250+fontSize*float64(number)))
	...
	engine.OnRegex(fmt.Sprintf(`^%s重置(所有|本群|/d+)?花名册$`, zero.BotConfig.CommandPrefix), zero.SuperUserPermission, getdb).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			var err error
...
func slicename(name string, canvas *images.ImageCtx) (resultname string) {
