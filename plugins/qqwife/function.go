...
func init() {
	engine.OnRegex(fmt.Sprintf(`^%s设置CD为?([\d]+(?:\.\d+)?)小时`, zero.BotConfig.CommandPrefix), zero.OnlyGroup, zero.AdminPermission, getdb).SetBlock(true).
	...
	engine.OnRegex(fmt.Sprintf(`^%s(允许|禁止)(自由恋爱|牛头人)$`, zero.BotConfig.CommandPrefix), zero.OnlyGroup, zero.AdminPermission, getdb).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
	...
	// 单身技能
	engine.OnMessage(zero.NewPattern(nil).Command(`^(娶|嫁)`).At().AsRule(), zero.OnlyGroup, getdb, checkSingleDog).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
	...
	// NTR技能
	engine.OnMessage(zero.NewPattern(nil).Command(`^当`).At().Text(`的小三`).AsRule(), zero.OnlyGroup, getdb, checkMistress).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			gid := ctx.Event.GroupID
			uid := ctx.Event.UserID
			patternParsed := ctx.State[zero.KeyPattern].([]zero.PatternParsed)
	...
	// 做媒技能
	engine.OnMessage(zero.NewPattern(nil).Command(`做媒`).At().At().AsRule(), zero.OnlyGroup, zero.AdminPermission, getdb, checkMatchmaker).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			gid := ctx.Event.GroupID
			uid := ctx.Event.UserID
			patternParsed := ctx.State[zero.KeyPattern].([]zero.PatternParsed)
	...
	engine.OnCommands([]string{"闹离婚", "办离婚"}, zero.OnlyGroup, getdb, checkDivorce).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			gid := ctx.Event.GroupID
			uid := ctx.Event.UserID
			// 写入CD
