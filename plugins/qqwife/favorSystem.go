...
func init() {
	// 好感度系统
	engine.OnMessage(zero.NewPattern(nil).Command(`查好感度`).At().AsRule(), zero.OnlyGroup, getdb).SetBlock(true).
	...
	// 礼物系统
	engine.OnMessage(zero.NewPattern(nil).Command(`买礼物给`).At().AsRule(), zero.OnlyGroup, getdb).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			...
			// 对接小熊饼干
			walletinfo := sc.BaseCoinOf(uid)
			if walletinfo < 1 {
				ctx.SendChain(message.Text("你钱包没钱啦！"))
				return
			}
			moneyToFavor := rand.Intn(math.Min(int(walletinfo), 100)) + 1
			// 计算钱对应的好感值
			newFavor := 1
			moodMax := 2
			if favor > 50 {
			...
			// 记录结果
			_, ok = sc.AddBaseCoin(uid, -float64(moneyToFavor))
			if !ok {
				ctx.SendChain(message.Text("[ERROR]:钱不够了！:"))
				return
			}
			...
			// 输出结果
			if mood == 0 {
				ctx.SendChain(message.Text("你花了", sc.RealCoin(float64(moneyToFavor)), sc.Unit(), "买了一件女装送给了ta,ta很不喜欢,你们的好感度降低至", lastfavor))
			} else {
				ctx.SendChain(message.Text("你花了", sc.RealCoin(float64(moneyToFavor)), sc.Unit(), "买了一件女装送给了ta,ta很喜欢,你们的好感度升至", lastfavor))
			}
		})
	engine.OnCommands([]string{"好感度列表"}, zero.OnlyGroup, getdb).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			uid := ctx.Event.UserID
			...
			fontSize := 50.0
			canvas := images.NewImageCtx(1150, int(170+(50+70)*float64(number)))
			...
			canvas.SetRGB(0, 0, 0)
			/***********设置字体大小,并获取字体高度用来定位***********/
			if err = canvas.UseDefaultFont(fontSize * 2); err != nil {
				ctx.SendChain(message.Text("[ERROR]:ERROR: ", err))
				return
			}
			...
			/***********设置字体大小,并获取字体高度用来定位***********/
			if err = canvas.UseDefaultFont(fontSize); err != nil {
				ctx.SendChain(message.Text("[ERROR]:ERROR: ", err))
				return
			}
			i := 0
	...
	engine.OnCommands([]string{"好感度数据整理"}, zero.SuperUserPermission, getdb).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
