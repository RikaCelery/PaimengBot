...
func drawstatus(uid int64, botname string, botrunstatus string) (sendimg image.Image, err error) {

	diskstate, err := diskstate()
	if err != nil {
		log.Warnln("获取磁盘状态失败: ", err)
		return
	}
	...
	bh, bw, ch, cw := float64(back.Bounds().Dy()), float64(back.Bounds().Dx()), float64(canvas.Height()), float64(canvas.Width())

	if bh/bw < ch/cw {
		back = imgfactory.Size(back, int(bw*ch/bh), int(bh*ch/bh)).Image()
		canvas.DrawImageAnchored(back, canvas.Width()/2, canvas.Height()/2, 0.5, 0.5)
	} else {
	...
	go func() {
		...
		titlecard.DrawRoundedRectangle(1, 1, float64(titlecard.Width()-1*2), float64(titlecardh-1*2), 16)
		...
		titlecard.LineTo(float64(titlecard.Width()-titlecardh), float64(titlecardh)/2)
		titlecard.Stroke()
	...
	go func() {
		...
		basiccard.DrawRoundedRectangle(1, 1, float64(basiccard.Width()-1*2), float64(basiccardh-1*2), 16)
		...
		for i, v := range basicstate {
			offset := float64(i) * ((float64(basiccard.Width())-200*float64(bslen))/float64(bslen+1) + 200)

			basiccard.SetRGBA255(57, 57, 57, 255)
			if isday() {
				basiccard.SetRGBA255(235, 235, 235, 255)
			}
			basiccard.DrawCircle((float64(basiccard.Width())-200*float64(bslen))/float64(bslen+1)+200/2+offset, 20+200/2, 100)
			basiccard.Fill()
			...
			basiccard.MoveTo((float64(basiccard.Width())-200*float64(bslen))/float64(bslen+1)+200/2+offset, 20+200/2)
			basiccard.DrawEllipticalArc((float64(basiccard.Width())-200*float64(bslen))/float64(bslen+1)+200/2+offset, 20+200/2, 100, 100, -0.5*math.Pi, -0.5*math.Pi+2*v.precent*0.01*math.Pi)
			basiccard.Fill()

			basiccard.SetColor(colorswitch(255))
			basiccard.DrawCircle((float64(basiccard.Width())-200*float64(bslen))/float64(bslen+1)+200/2+offset, 20+200/2, 80)
			basiccard.Fill()
			...
			basiccard.DrawStringAnchored(strconv.FormatFloat(v.precent, 'f', 0, 64)+"%", (float64(basiccard.Width())-200*float64(bslen))/float64(bslen+1)+200/2+offset, 20+200/2, 0.5, 0.5)

			basiccard.SetColor(fontcolorswitch())

			_, fw := basiccard.MeasureString(v.name)
			basiccard.DrawStringAnchored(v.name, (float64(basiccard.Width())-200*float64(bslen))/float64(bslen+1)+200/2+offset, 20+200+15+basiccard.FontHeight()/2, 0.5, 0.5)
			...
			for k, s := range v.text {
				basiccard.DrawStringAnchored(s, (float64(basiccard.Width())-200*float64(bslen))/float64(bslen+1)+200/2+offset, 20+200+15+fw+15+basiccard.FontHeight()/2+float64(k)*textoffsety, 0.5, 0.5)
			}
		}
	...
	go func() {
		...
		diskcard.DrawRoundedRectangle(1, 1, float64(diskcard.Width()-1*2), float64(diskcardh-1*2), 16)
		...
		if dslen == 1 {
			...
			diskcard.DrawRoundedRectangle(40, 40, float64(diskcard.Width())-40-100, 50, 12)
			diskcard.ClipPreserve()
			diskcard.Fill()
			...
			diskcard.DrawRoundedRectangle(40, 40, (float64(diskcard.Width())-40-100)*diskstate[0].precent*0.01, 50, 12)
			diskcard.Fill()
			diskcard.ResetClip()
			...
			diskcard.DrawStringAnchored(diskstate[0].text[0], (float64(diskcard.Width())-100-10)-fw1/2, 40+50/2, 0.5, 0.5)
			diskcard.DrawStringAnchored(strconv.FormatFloat(diskstate[0].precent, 'f', 0, 64)+"%", float64(diskcard.Width())-100/2, 40+50/2, 0.5, 0.5)
		} else {
			for i, v := range diskstate {
				...
				diskcard.DrawRoundedRectangle(40, 40+(float64(diskcardh-40*2)-50*float64(dslen))/float64(dslen-1)+offset, float64(diskcard.Width())-40-100, 50, 12)
				diskcard.ClipPreserve()
				diskcard.Fill()
				...
				diskcard.DrawRoundedRectangle(40, 40+(float64(diskcardh-40*2)-50*float64(dslen))/float64(dslen-1)+offset, (float64(diskcard.Width())-40-100)*v.precent*0.01, 50, 12)
				diskcard.Fill()
				diskcard.ResetClip()
				...
				diskcard.DrawStringAnchored(v.text[0], (float64(diskcard.Width())-100-10)-fw1/2, 40+(float64(diskcardh-40*2)-50*float64(dslen))/float64(dslen-1)+50/2+offset, 0.5, 0.5)
				diskcard.DrawStringAnchored(strconv.FormatFloat(v.precent, 'f', 0, 64)+"%", float64(diskcard.Width())-100/2, 40+(float64(diskcardh-40*2)-50*float64(dslen))/float64(dslen-1)+50/2+offset, 0.5, 0.5)
			}
		}
	...
	go func() {
		...
		moreinfocard.DrawRoundedRectangle(1, 1, float64(moreinfocard.Width()-1*2), float64(moreinfocard.Height()-1*2), 16)
		...
		for i, v := range moreinfo {
			...
			moreinfocard.DrawStringAnchored(v.text[0], float64(moreinfocard.Width())-20-fw1/2, 30+(float64(moreinfocardh-30*2)-moreinfocard.FontHeight()*float64(milen))/float64(milen-1)+moreinfocard.FontHeight()/2+offset, 0.5, 0.5)
		}
		moreinfoimg = moreinfocard.Image()
	}()
	go func() {
		defer wg.Done()
		shadow := gg.NewContext(canvas.Width(), canvas.Height())
	...
	canvas.DrawStringAnchored("Created By ZeroBot-Plugin "+"#e9184c12", float64(canvas.Width())/2+3, float64(canvas.Height())-70/2+3, 0.5, 0.5)
	canvas.SetRGBA255(255, 255, 255, 255)
	canvas.DrawStringAnchored("Created By ZeroBot-Plugin "+"#e9184c12", float64(canvas.Width())/2, float64(canvas.Height())-70/2, 0.5, 0.5)
