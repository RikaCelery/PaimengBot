...
func init() {
	engine := manager.RegisterPlugin(manager.PluginInfo{
		Name: "quote",
		Usage: `用法：
	[回复]/q  [消息数量][参数]->渲染一张带有被回复人和话的大照片
消息数量：
	0：单张图片
	1：渲染为历史消息记录
参数：
	-g：灰度滤镜，默认关闭(仅Size为0时生效)
	-d：包含时间日期，默认关闭(仅Size为0时生效)
例子：
	[回复]/q
	[回复]/q 5：从被回复消息开始查找5条消息渲染
	[回复]/q -g：灰色滤镜
...
`,
	...
	engine.OnMessage(zero.NewPattern(&zero.PatternOption{CleanRedundantAt: true}).Reply().Text(`/q\s*(.*)`).AsRule(), zero.OnlyGroup).SetBlock(true).
